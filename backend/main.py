...
async def lifespan(app: FastAPI):
    ...
    # ① DB Schema 自动迁移（幂等，每次启动执行）。核心表迁移失败时终止启动，
    # 避免应用在 Schema 不完整的情况下继续接收请求。
    from backend.db.migrations import run_migrations
    await run_migrations()

    # ② 初始化 LangGraph 状态持久化。默认 memory 不需要数据库表；生产环境
    # 通过 LANGGRAPH_CHECKPOINTER=postgres 切换到持久化检查点。
    from backend.core.memory import initialize_postgres_checkpointer, close_postgres_checkpointer
    try:
        postgres_enabled = await initialize_postgres_checkpointer(settings.database_url)
        if postgres_enabled:
            # API 模块可能在 lifespan 前已编译图，切换后重新编译以绑定 PostgresSaver。
            import backend.api.v1.exam as exam_api
            ...
            exam_api._graph = build_exam_graph()
            interview_api._graph = build_interview_graph()
    except Exception:
