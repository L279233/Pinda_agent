...
class InterviewReport(BaseModel):
    ...
    overall_score:      int       = Field(ge=0, le=100, description="综合得分 0-100（五维度加权平均）")
    ...
    evidence:           list[str] = Field(default_factory=list, description="面试回答中的关键证据")
    risk_flags:         list[str] = Field(default_factory=list, description="矛盾、缺失或待核验事项")
