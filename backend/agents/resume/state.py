...
class ResumeSummary(BaseModel):
    ...
    core_improvements: list[str] = Field(description="2-3 条主要缺口")
    overall_comment:   str       = Field(description="1-2 句综合评语")
    fit_assessment:    str       = Field(description="对目标岗位的匹配度评估（1句话）")
    decision: Literal["pass", "reject", "manual_review"] = Field(
        description="模型给出的辅助建议，最终结论由业务规则决定"
    )
    evidence: list[str] = Field(default_factory=list, description="来自简历原文的匹配证据")
    risk_flags: list[str] = Field(default_factory=list, description="信息缺失、矛盾或待核验项")
    manual_review_required: bool = Field(
        default=False, description="是否建议招聘官人工复核"
    )
...
class ResumeState(TypedDict):
    ...
    application_id: str        # recruitment_applications 表的 UUID
    position_id:    str
    position_title: str
    job_description: str
