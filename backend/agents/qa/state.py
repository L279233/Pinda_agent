...
class QAState(TypedDict):
    ...
    student_id:  str            # 候选人 ID（保留旧字段名兼容现有表）
    tenant_id:   str            # 租户 ID（Milvus / DB 多租户隔离）
    session_id:  str            # 会话 ID（用于构造 thread_id）
    position_id: Optional[str]  # 岗位 ID，限制招聘知识检索范围；None = 租户公共资料
    application_id: Optional[str]  # 招聘申请 ID，仅用于查询候选人本人任务
    candidate_task_context: Optional[dict]  # API 校验后注入的候选人可见任务数据

    # ── ③ Query 处理中间结果──────────────────────────────────────
    original_query:    str         # 用户原始输入，全程不变
    query_type:        str         # GENERAL / PRECISE / VAGUE / BROAD
    question_scope:    str         # general / recruitment / application_state / private_result
