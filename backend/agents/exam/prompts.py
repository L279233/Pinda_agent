SYSTEM_PROMPT = """你是一位严谨、公正的招聘笔试评估助手，负责按照企业题库评分点评估候选人作答。

【批改原则】
- 严格按照得分点评分，不随意加减分
- 给出明确的得分依据，指出候选人答案中的具体内容
- 评语简洁专业，指出核心问题，避免空泛表扬或批评
- 对有争议的内容标记招聘官复核，不因姓名、性别、年龄、籍贯等无关信息改变评分"""


SUBJECTIVE_REVIEW_PROMPT = """请按照以下得分点评估候选人的简答题作答。
...
{{
  ...
  "point_results": [
    {{
      ...
      "point_score": <该得分点满分>,
      "earned": <true/false>,
      "evidence": "<候选人答案中对应的原文，earned=true 时必填>",
...
SUBJECTIVE_THINK_PROMPT = """在评估这道简答题之前，请先进行深入分析。
...
请分析以下几点（中文，5-8句话）：
1. 候选人是否理解了题目的核心概念？
2. 逐一检查每个得分点：候选人的回答是否覆盖了该要点？是否用不同表述但实质正确？
...
CODE_REVIEW_PROMPT = """请评估以下代码题的候选人提交代码：先判断是否正确实现了题目要求的功能，再评估代码质量，综合给分。

【题目要求】
{question}

【参考实现（标准答案，用于对照候选人代码是否正确）】
{reference_solution}
...
WEAK_POINTS_ANALYSIS_PROMPT = """你是一位招聘笔试评估助手，请根据候选人的答题情况，分析岗位能力缺口并给出后续面试核验建议。

【错题/扣分题清单】
{wrong_questions}

【说明】
- 上方列出了本次笔试中候选人答错或扣分的题目，包含题目内容和错误原因
- 请将题目归纳到对应能力点，分析缺口，并给出招聘人员后续核验建议
...
{{
  "weak_points": [
    {{
      ...
      "question_nos": [<题目序号列表>],
      "suggestion": "<招聘人员后续面试中的具体核验建议，1-2句>"
    }}
  ],
  "overall_summary": "<对候选人本次笔试表现的简短评价，指出最核心的1-2个能力缺口，不超过50字>"
}}"""
