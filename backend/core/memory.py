...
_postgres_savers: dict[str, Any] = {}
_postgres_stack: AsyncExitStack | None = None


def checkpointer_backend() -> str:
    """Return the configured backend without importing optional psycopg modules."""
    from backend.config import get_settings

    configured = os.getenv(
        "LANGGRAPH_CHECKPOINTER", get_settings().langgraph_checkpointer
    )
    return configured.strip().lower() or "memory"
...
async def initialize_postgres_checkpointer(database_url: str) -> bool:
    """Initialize a process-scoped PostgresSaver for production deployments.

    Keeping the async context open for the process lifetime is required; closing it
    would invalidate the saver held by compiled graphs.
    """
    global _postgres_stack
    backend = checkpointer_backend()
    if backend == "memory":
        return False
    if backend != "postgres":
        raise ValueError(
            "LANGGRAPH_CHECKPOINTER 仅支持 'memory' 或 'postgres'"
        )
    if _postgres_stack is not None:
        return True
    AsyncPostgresSaver = _load_async_postgres_saver()
    ...
    try:
        ...
        await stack.aclose()
        raise
    _postgres_stack = stack
    _postgres_savers["default"] = saver
    logger.info("memory.postgres_checkpointer_initialized")
    return True


async def close_postgres_checkpointer() -> None:
    """Release the process-scoped Postgres connection on application shutdown."""
    global _postgres_stack
    if _postgres_stack is not None:
        await _postgres_stack.aclose()
        _postgres_stack = None
        _postgres_savers.clear()
        logger.info("memory.postgres_checkpointer_closed")


def get_memory_saver(agent_type: str = "default") -> Any:
    """
    ...
    """
    if _postgres_stack is not None:
        # Postgres uses one concurrency-safe saver; UUID session IDs isolate graph state.
        return _postgres_savers["default"]
...
def build_config(
    student_id: str, session_id: str, agent_type: str | None = None
) -> dict:
    """
    ...
    if agent_type:
        thread_id = f"{agent_type}:{thread_id}"
    return {
        "configurable": {
            "thread_id": thread_id,
        }
