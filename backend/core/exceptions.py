...
class RecruitmentError(EduAgentBaseError):
    """招聘申请领域错误的基类，均属于不可重试的业务异常。"""
    pass


class ApplicationNotFoundError(RecruitmentError):
    """岗位或招聘申请不存在，或不属于当前租户。"""
    pass


class ApplicationPermissionError(RecruitmentError):
    """当前用户无权操作该招聘申请。"""
    pass


class ApplicationStateError(RecruitmentError):
    """招聘申请当前状态不允许执行目标动作。"""
    pass
