...
from backend.core.memory import build_config                       # 5.10 的 thread_id 工具
...
_AGENT_DISPLAY = {
    AgentType.QA:        "智能问答",
    AgentType.EXAM:      "在线笔试",
    AgentType.RESUME:    "简历投递",
    AgentType.INTERVIEW: "AI 面试",
}
...
_GUIDANCE = {
    AgentType.EXAM: {
        "message": "检测到您要参加在线笔试。请从候选人任务中心进入已开放的笔试任务。",
        "action_label": "前往任务中心",
        "action_url": "/applications",
    },
    AgentType.RESUME: {
        "message": "检测到您有应聘意向。请先选择招聘岗位并创建申请，再按任务提示上传简历。",
        "action_label": "查看招聘岗位",
        "action_url": "/positions",
    },
    AgentType.INTERVIEW: {
        "message": "检测到您要参加 AI 面试。请从候选人任务中心进入已开放的面试任务。",
        "action_label": "前往任务中心",
        "action_url": "/applications",
    },
}
...
_QA_MODEL_UNAVAILABLE_MESSAGE = (
    "已完成招聘资料检索，但当前智能问答模型暂时无法连接。"
    "请稍后重试；如果问题紧急，请联系 HR 获取准确答复。"
)
# ── 去除末尾标点空白，用于关键词精确比对 ──────────────────────────
...
_REPLY_HELLO = (
    "您好！我是聘达招聘助手。\n\n"
    "我可以帮您：\n"
    "- **招聘答疑**：查询岗位要求、招聘制度和招聘流程\n"
    "- **简历投递**：从任务中心进入对应申请并上传简历\n"
    "- **在线笔试与 AI 面试**：任务开放后从任务中心进入\n"
    + _MULTI_AGENT_TIP
    + "\n请问有什么可以帮到您？"
)
...
_REPLY_BYE = (
    "再见！招聘安排请以任务中心和 HR 的正式通知为准。"
)

_REPLY_IDENTITY = (
    "我是 **聘达招聘助手**，面向候选人提供招聘流程服务。\n\n"
    "我由多个专业 Agent 协同构成：\n"
    "- **招聘答疑 Agent**：基于招聘资料回答岗位和流程问题\n"
    "- **简历、笔试、面试 Agent**：处理招聘初筛任务\n"
    + _MULTI_AGENT_TIP
    + "\n有什么可以帮到您吗？"
)
...
_REPLY_CAPABILITY = (
    "我能为您提供以下功能：\n\n"
    "- 直接输入岗位或流程问题 → 招聘资料检索答疑\n"
    "- 投递简历、参加笔试或 AI 面试 → 从候选人任务中心进入\n"
    "- 评分、报告和招聘结果 → 等待 HR 正式通知\n\n"
    + _MULTI_AGENT_TIP
...
可选功能：
- qa          : 岗位、公司、招聘流程或通用知识问答
- exam        : 参加在线笔试或提交答卷
- resume      : 投递或上传简历
- interview   : 参加 AI 面试
- multi_agent : 同时涉及简历、笔试、面试等多个招聘环节
...
_INTERVIEW_START_RE = re.compile(r"(开始|进入|参加|进行|回答|下一题|提交).{0,8}面试|面试.{0,8}(开始|进入|回答|下一题)")
_INTERVIEW_INFO_RE = re.compile(r"面试.*(流程|规则|考察|评分|内容|问题|准备|注意|介绍|是什么|怎么|哪些)")


def _keyword_route(message: str) -> tuple[str, str]:
    """LLM 路由不可用时的轻量兜底，保证常见操作仍能进入专用 Agent。"""
    text = message.lower()
    asks_for_information = re.search(
        r"流程|要求|条件|信息|介绍|是什么|怎么|如何|哪些|注意事项", text
    )
    if re.search(r"简历|投递|应聘|求职|申请岗位", text) and re.search(r"笔试|面试", text):
        return "multi_agent", "识别到多个招聘环节，进入求职流程协同"
    if re.search(r"(我要|我想|准备|希望|打算)?(应聘|求职|申请岗位|找工作)", text) and not asks_for_information:
        return "resume", "识别到应聘意向，转入招聘申请流程"
    if re.search(r"简历|投递|上传材料|个人资料", text) and not asks_for_information:
        return "resume", "识别到简历投递需求，转入简历 Agent"
    if re.search(r"笔试|试卷|答题|考试|提交答案", text):
        return "exam", "识别到笔试需求，转入笔试 Agent"
    if re.search(r"面试|初面|模拟面试", text) and not _INTERVIEW_INFO_RE.search(message):
        return "interview", "识别到面试需求，转入面试 Agent"
    return "qa", "未识别到专用操作，转入智能问答"
...
async def _llm_route(message: str) -> _RouteResult:
    ...
    """
    # 明确的操作型意图优先本地识别，避免模型不可用或误判时进入 QA Agent。
    direct_label, direct_reason = _keyword_route(message)
    if direct_label != "qa":
        return _RouteResult(
            label=direct_label,
            agent_type=_LABEL_TO_AGENT[direct_label],
            execution_mode=_LABEL_TO_MODE[direct_label],
            confidence=0.95,
    ...
    try:
        ...
        if label == "interview" and _INTERVIEW_INFO_RE.search(message) and not _INTERVIEW_START_RE.search(message):
            label = "qa"
            reason = f"面试咨询转 QA Agent 使用 RAG；原始判断：{reason}"
        logger.info("unified_chat.llm_route_result", label=label, reason=reason)

    except Exception as e:                            # 调用/解析失败 → 关键词兜底，不阻断流
        label, reason = _keyword_route(message)
        logger.warning("unified_chat.llm_route_failed", error=str(e), fallback=label)
...
class UnifiedChatRequest(BaseModel):
    ...
    application_id: uuid.UUID | None = Field(None, description="招聘申请 ID")
    position_id: uuid.UUID | None = Field(None, description="岗位 ID")
    enable_web_search: bool = Field(False, description="通用问题是否允许联网补充")
    ...
    async def event_generator():                      # 异步生成器：逐个 yield SSE 事件
        ...
        # ── 3c：multi_agent → 求职全链路 Pipeline 计划 ──────
            ...
            yield _sse({
                "type":     "pipeline_plan",
                "title":    "求职全链路",
                "intro":    "招聘任务按申请状态依次开放，请从候选人任务中心进入对应任务。",
                "steps": [
                    {
                        "step":         1,
                        "agent_type":   "resume",
                        "label":        "简历投递",
                        "desc":         "在对应招聘申请中上传 PDF 简历",
                        "action_label": "前往任务中心",
                        "action_url":   "/applications",
                        "tip":          "提交后请等待 HR 通知后续安排",
                    },
                    {
                        "step":         2,
                        "agent_type":   "interview",
                        "label":        "笔试与 AI 面试",
                        "desc":         "任务开放后，在规定时间内完成对应环节",
                        "action_label": "前往任务中心",
                        "action_url":   "/applications",
                        "tip":          "候选人端不展示评分或评估报告",
                    },
        ...
        # ── 3d：clarify → 追问提示 ───────────────────────────
            ...
            yield _sse({
                ...
                "message": "您的问题我还不太确定应该用哪个功能来帮您，能否描述得更具体一些？"
                           "例如：您是想咨询岗位、投递简历、参加笔试，还是进行 AI 面试？",
                "action_label": "",
                "action_url":   "",
            })
...
async def _stream_qa_agent(req: UnifiedChatRequest, current_user: dict):
    ...
    from backend.api.v1.qa import ChatRequest, build_initial_state, resolve_qa_context

    orchestrator = get_orchestrator()
    graph = orchestrator._get_agent_graph(AgentType.QA)       # 复用编排器缓存的 QA 图（不重复 build）

    thread_id = build_config(current_user["user_id"], req.session_id, "qa")["configurable"]["thread_id"]  # 拼检查点 key
    qa_req = ChatRequest(**req.model_dump())
    try:
        position_id, task_context = await resolve_qa_context(qa_req, current_user)
    except Exception as exc:
        detail = getattr(exc, "detail", "招聘申请或岗位信息无效")
        yield _sse({"type": "error", "message": str(detail)})
        return
    initial_state = build_initial_state(qa_req, current_user, position_id, task_context)
    ...
    try:
        ...
        async for event in graph.astream_events(initial_state, config=config, version="v2"):
            ...
            # ③ 生成节点结束 → 回填 answer_mode / sources / confidence（供最后 meta 用）
                ...
                if output.get("answer") and not emitted_answer:
                    emitted_answer = True
                    yield _sse({"type": "token", "content": output["answer"]})
        ...
        # 检索和模型生成是两个阶段。模型网络异常时，仍返回可理解的降级消息，
        # 避免候选人只看到“请求失败”而不知道资料是否已提交或服务是否可用。
        logger.error("unified_chat.qa_stream_error", error=str(e), exc_info=True)
        yield _sse({"type": "token", "content": _QA_MODEL_UNAVAILABLE_MESSAGE})
        yield _sse({
            "type": "meta",
