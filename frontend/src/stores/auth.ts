...
interface UserInfo {
  userId: string
  role: 'student' | 'teacher' | 'admin' | 'hr' | 'recruiter'
  tenantId: string
  username?: string
}
...
export const useAuthStore = defineStore('auth', () => {
  ...
  const isCandidate = computed(() => user.value?.role === 'student')
  const isTeacher = computed(() =>
    ['teacher', 'admin', 'hr', 'recruiter'].includes(user.value?.role || '')
  )
  ...
  return { token, user, isLoggedIn, isCandidate, isTeacher, login, logout }
})
