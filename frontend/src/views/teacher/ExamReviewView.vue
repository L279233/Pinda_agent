  ...
  <div class="exam-review">
    <el-card>
      ...
      <el-table :data="list" v-loading="loading" size="default">
        <el-table-column prop="student_name" label="候选人" width="120" />
        <el-table-column prop="exam_title" label="试卷" />
        <el-table-column prop="submitted_at" label="提交时间" width="180" />
        <el-table-column label="AI 预评分" width="120">
          <template #default="{ row }">
            <span v-if="hasReviewSummary(row)">{{ row.pre_review.total_score }} / {{ row.pre_review.full_score }}</span>
            <span v-else class="unavailable">历史数据缺失</span>
          </template>
        </el-table-column>
        <el-table-column label="待确认题数" width="100">
          <template #default="{ row }">
            <el-tag v-if="hasReviewSummary(row)" type="warning" size="small">{{ row.pre_review.needs_review_count }}</el-tag>
            <span v-else>-</span>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="100">
          <template #default="{ row }">
            <el-button type="primary" text size="small" :disabled="!hasReviewSummary(row)" :title="hasReviewSummary(row) ? '审阅笔试' : '历史数据不可恢复，请候选人重新提交'" @click="openReview(row.submission_id)">
              审阅
            </el-button>
          </template>
        </el-table-column>
    ...
    <!-- 审阅抽屉 -->
    <el-drawer v-model="drawerVisible" title="招聘官复核" size="65%" direction="rtl" destroy-on-close>
          ...
          >
            ...
            <div class="q-detail">
              ...
              <div class="q-modify-row">
                <span class="q-label" style="min-width: 72px">招聘官改分</span>
                <el-input-number
...
function hasReviewSummary(row: PendingReviewItem) {
  return Number.isFinite(row.pre_review?.total_score) && Number.isFinite(row.pre_review?.full_score)
}
...
async function fetchList() {
  ...
  try {
    ...
    ElMessage.error(e?.response?.data?.detail ?? '待复核列表加载失败')
  } finally {
    loading.value = false
  }
}
...
async function openReview(submissionId: string) {
  try {
    ...
    if (!data.pre_review_summary?.by_question?.length) {
      ElMessage.warning('该历史记录没有可恢复的批改明细，请候选人重新提交笔试')
      return
    }
  ...
  expandedKeys.value = (data.pre_review_summary?.by_question ?? [])
    ...
    ElMessage.error(e?.response?.data?.detail ?? '笔试批改明细加载失败')
  }
}
...
onMounted(async () => {
  await fetchList()
  const submissionId = String(route.query.submission_id || '')
  const selected = list.value.find(item => item.submission_id === submissionId)
  if (selected) {
    if (hasReviewSummary(selected)) await openReview(submissionId)
    else ElMessage.warning('该历史记录没有可恢复的批改明细，请候选人重新提交笔试')
  }
})
</script>
...
.unavailable { color: #909399; font-size: 12px; }
</style>
