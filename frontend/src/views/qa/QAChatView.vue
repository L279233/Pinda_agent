    ...
    <div class="chat-panel">
      ...
      <div class="chat-input-area">
        ...
        <div class="input-actions">
          <el-tooltip
            :content="webSearchEnabled ? '通用问题可使用互联网信息；企业招聘信息仍以招聘资料为准' : '已关闭互联网信息补充'"
            placement="top"
          >
            <el-switch
...
// Use the current frontend origin by default so requests go through the Vite/Nginx proxy.
// This avoids localhost/127.0.0.1 CORS and preflight differences.
...
const answerModeLabel = computed(() => {
  switch (lastAnswerMode.value) {
    case 'rag':           return '招聘资料'
    case 'web_augmented': return '互联网参考'
    case 'general':       return '通用回答'
    case 'controlled':    return '申请信息'
    case 'restricted':    return '请联系 HR'
    default:              return '通用回答'
  }
})
  ...
  try {
    ...
    const resp = await fetch(`${API_BASE}/api/v1/qa/chat/stream`, {
      ...
      body: JSON.stringify({
        ...
        application_id:    route.query.application_id || null,
    ...
    if (!resp.ok || !resp.body) {
      ...
      throw new Error(detail || `招聘答疑请求失败（HTTP ${resp.status}）`)
    }
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        if (evt.type === 'progress' && evt.stage) {
          ...
          throw new Error(evt.message ?? '招聘答疑流式响应异常')
        }
      }
    }
    ...
    ElMessage.error(err instanceof Error ? err.message : '请求失败，请重试')
